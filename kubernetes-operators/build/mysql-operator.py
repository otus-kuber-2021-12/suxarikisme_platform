...
def mysql_on_create(body, spec, **kwargs):
   ...
   # Создаем mysql PV:
    ...
    try:
        ...
        api = kubernetes.client.CoreV1Api()
        print(api.create_namespaced_persistent_volume_claim('default',backup_pvc))
    #api.create_namespaced_persistent_volume_claim('default', backup_pvc)
    except kubernetes.client.rest.ApiException:
        pass

#UPDATING Status Object
#Requires x-kubernetes-preserve-unknown-fields: true on CRD
@kopf.on.create('otus.homework', 'v1', 'mysqls')
def Kopf(uid, **_):
    return {'Created': "I know Python!"}

#UPDATE hook for password
#Only for deployment
...
def update_fn(body, spec, **kwargs):
    ...
    api = kubernetes.client.AppsV1Api()
    api_del = kubernetes.client.AppsV1Api()
    api_del.delete_namespaced_deployment(name,'default')
    api.create_namespaced_deployment('default', deployment)
